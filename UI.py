...
def startScanning():
    ...
    #     print("Starting scan, instance " + str(scan_count) + "...")
        subprocess.run(['python', 'sensors/realsense_recorder.py', '--record_imgs'], shell=True, capture_output=True)        
    # else:
...
def deleteLastScan():
    scan_count = len([f for f in os.listdir(path)if os.path.isfile(os.path.join(path, f))])
    if (scan_count >0):
        text_scan_path = 'dataset/realsense/scan/scan'
        if not exists(text_scan_path):
            makedirs(text_scan_path)
